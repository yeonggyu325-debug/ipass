(function(global){
  ...
  if((location.pathname==='/'||location.pathname==='/index.html')&&global.EHSAuth?.readSession?.()){
  ...
  async function request(path,options={},retry=true){
    ...
    if(response.status===401&&retry&&global.EHSAuth?.readSession()){
      try{await global.EHSAuth.refresh();return request(path,options,false)}catch(refreshError){authFailed(refreshError);throw refreshError}
    }
  ...
  async function authorizedBlob(path,retry=true){
    ...
    if(global.EHSAuth?.readSession()&&isTrustedApiUrl(url)){try{headers.set('Authorization','Bearer '+await global.EHSAuth.token())}catch(error){authFailed(error);throw error}}
    let response;
  ...
  if(!document.querySelector('link[data-global-toolbar-v5]')){const link=document.createElement('link');link.rel='stylesheet';link.href='/global-toolbar-v5.css?v=1';link.dataset.globalToolbarV5='true';document.head.appendChild(link)}
  if(!document.querySelector('script[data-global-toolbar-v5]')){const script=document.createElement('script');script.src='/global-toolbar-v5.js?v=1';script.dataset.globalToolbarV5='true';document.head.appendChild(script)}

  if(!document.querySelector('link[data-portal-home-v3]')){const link=document.createElement('link');link.rel='stylesheet';link.href='/portal-home-v3.css?v=2';link.dataset.portalHomeV3='true';document.head.appendChild(link)}
  if(!document.querySelector('script[data-portal-home-v3]')){const script=document.createElement('script');script.src='/portal-home-v3.js?v=2';script.dataset.portalHomeV3='true';document.head.appendChild(script)}
