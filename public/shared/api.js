(function(global){
  ...
  async function request(path,options={},retry=true){
    const url=/^https?:\/\//.test(path)?path:ORIGIN+path;const {timeoutMs,...fetchOptions}=options;const headers=new Headers(options.headers||{});headers.set('Accept',headers.get('Accept')||'application/json');const body=options.body;const isForm=typeof FormData!=='undefined'&&body instanceof FormData;if(body&&!isForm&&!headers.has('content-type'))headers.set('content-type','application/json');
    if(global.EHSAuth&&isTrustedApiUrl(url)){const session=global.EHSAuth.readSession();if(session){try{headers.set('Authorization','Bearer '+await global.EHSAuth.token())}catch(error){authFailed(error);throw error}}}
    let response;try{response=await timedFetch(url,{...fetchOptions,headers},timeoutMs||(isForm?60000:DEFAULT_TIMEOUT_MS))}catch(error){if(error?.code==='REQUEST_TIMEOUT')throw error;throw makeError('서버에 연결할 수 없습니다. 잠시 후 다시 시도해 주세요.',0,'NETWORK_ERROR',null,{cause:error})}
    const data=await parse(response);const requestId=response.headers.get('x-request-id')||data.request_id||null;
    if(response.status===401&&retry&&global.EHSAuth?.readSession()){try{await global.EHSAuth.refresh();return request(path,options,false)}catch(refreshError){authFailed(refreshError);throw refreshError}}
    if(!response.ok){const message=data.error||data.message||`요청 처리 중 오류가 발생했습니다. (HTTP ${response.status})`;const error=makeError(message,response.status,data.code||data.error_code,requestId,data);authFailed(error);throw error}
    return data;
  }

  async function authorizedBlob(path,retry=true){
    const url=/^https?:\/\//.test(path)?path:ORIGIN+path;const headers=new Headers();if(global.EHSAuth?.readSession()&&isTrustedApiUrl(url)){try{headers.set('Authorization','Bearer '+await global.EHSAuth.token())}catch(error){authFailed(error);throw error}}
    let response;try{response=await timedFetch(url,{headers},30000)}catch(error){if(error?.code==='REQUEST_TIMEOUT')throw error;throw makeError('파일 서버에 연결할 수 없습니다.',0,'NETWORK_ERROR')}
    if(response.status===401&&retry&&global.EHSAuth?.readSession()){try{await global.EHSAuth.refresh();return authorizedBlob(path,false)}catch(error){authFailed(error);throw error}}
    if(!response.ok){const data=await parse(response);const error=makeError(data.error||'파일을 다운로드할 수 없습니다.',response.status,data.code,response.headers.get('x-request-id'),data);authFailed(error);throw error}
  ...
  if(location.pathname.startsWith('/ipass')){
    ...
    if(!document.querySelector('script[data-ipass-ui-v2]')){const script=document.createElement('script');script.src='/ipass-ui-v2.js?v=1';script.dataset.ipassUiV2='true';document.head.appendChild(script)}
  }
  if(location.pathname==='/evaluation-submit.html'){
    if(!document.querySelector('link[data-submission-nav-v2]')){const link=document.createElement('link');link.rel='stylesheet';link.href='/evaluation-submit-nav-v2.css?v=1';link.dataset.submissionNavV2='true';document.head.appendChild(link)}
    if(!document.querySelector('script[data-submission-nav-v2]')){const script=document.createElement('script');script.src='/evaluation-submit-nav-v2.js?v=1';script.dataset.submissionNavV2='true';document.head.appendChild(script)}
  }
})(window);
