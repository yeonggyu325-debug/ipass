...
async function bundle(env,targetId){
  const [targetResult,itemResult,fileResult,logResult]=await env.partner_evaluation_db.batch([
    ...
    env.partner_evaluation_db.prepare(`SELECT id,target_item_id,file_name,content_type,file_size,created_at FROM evaluation_evidence_files_v2 WHERE target_id=? AND deleted_at IS NULL ORDER BY created_at DESC`).bind(targetId),
    env.partner_evaluation_db.prepare(`SELECT action,target_item_id,before_json,after_json,changed_by,created_at FROM evaluation_scoring_logs_v2 WHERE target_id=? ORDER BY created_at DESC LIMIT 50`).bind(targetId)
  ]);
  const t=targetResult.results?.[0];if(!t)return null;const rows=itemResult.results||[];
...
export async function handleEvaluationScoring(request,env,ctx,innerApp){
  ...
  if(itemRoute&&request.method==='PATCH'){
    ...
    const [targetResult,itemResult]=await env.partner_evaluation_db.batch([
      env.partner_evaluation_db.prepare(`SELECT et.*,c.company_name,c.industry_name,ec.year,ec.half,ec.cycle_name,ec.status AS cycle_status,ec.start_at,ec.end_at FROM evaluation_targets_v2 et JOIN companies c ON c.id=et.company_id JOIN evaluation_cycles_v2 ec ON ec.id=et.cycle_id WHERE et.id=? AND et.is_selected=1 LIMIT 1`).bind(targetId),
      env.partner_evaluation_db.prepare(`SELECT * FROM evaluation_target_items_v2 WHERE id=? AND target_id=? LIMIT 1`).bind(itemId,targetId)
    ]),t=targetResult.results?.[0],row=itemResult.results?.[0];if(!t)return json({success:false,error:'평가대상을 찾을 수 없습니다.',code:'TARGET_NOT_FOUND'},404);
    ...
    if(!row)return json({success:false,error:'평가항목을 찾을 수 없습니다.',code:'ITEM_NOT_FOUND'},404);
    if(row.applicability_status==='undetermined')return json({success:false,error:'적용대상 판정이 완료되지 않은 항목은 채점할 수 없습니다.',code:'ITEM_APPLICABILITY_UNDETERMINED'},409);
    ...
    await env.partner_evaluation_db.batch([
      ...
      env.partner_evaluation_db.prepare(`UPDATE evaluation_targets_v2 SET status=CASE WHEN status='submitted' THEN 'evaluating' ELSE status END,raw_score=(SELECT ROUND(MIN(100,MAX(0,CASE WHEN COALESCE(SUM(CASE WHEN applicability_status='applicable' AND item_type<>'bonus' THEN max_score ELSE 0 END),0)>0 THEN COALESCE(SUM(CASE WHEN applicability_status='applicable' AND item_type<>'bonus' THEN earned_score ELSE 0 END),0)*100.0/SUM(CASE WHEN applicability_status='applicable' AND item_type<>'bonus' THEN max_score ELSE 0 END) ELSE 0 END+COALESCE(SUM(CASE WHEN applicability_status='applicable' AND item_type='bonus' THEN earned_score ELSE 0 END),0))),1) FROM evaluation_target_items_v2 WHERE target_id=?),updated_at=CURRENT_TIMESTAMP WHERE id=?`).bind(targetId,targetId),
      env.partner_evaluation_db.prepare(`INSERT INTO evaluation_scoring_logs_v2 (id,target_id,target_item_id,action,before_json,after_json,changed_by) VALUES (?,?,?,?,?,?,?)`).bind(crypto.randomUUID(),targetId,itemId,action,JSON.stringify(before||null),JSON.stringify({earned_score:score,evaluation_comment:comment,needs_rescore:false}),auth.user.id)
    ]);
    const [updatedItem,updatedTarget,scoreRows]=await env.partner_evaluation_db.batch([
    ...
    return json({success:true,item:updatedItem.results?.[0]||null,target:updatedTarget.results?.[0]||null,summary:scoreSummary(scoreRows.results||[])});
  }

  const complete=path.match(/^\/api\/admin\/evaluation-scoring\/([^/]+)\/complete$/);
  if(complete&&request.method==='POST'){
    const targetId=decodeURIComponent(complete[1]);await reconcileTargetApplicability(env,targetId);const data=await bundle(env,targetId);if(!data)return json({success:false,error:'평가대상을 찾을 수 없습니다.',code:'TARGET_NOT_FOUND'},404);
    ...
    if(data.summary.undetermined_count>0)return json({success:false,error:`적용대상 판정이 필요한 항목 ${data.summary.undetermined_count}개가 남아 있습니다.`,code:'APPLICABILITY_UNDETERMINED',summary:data.summary},409);
  ...
  if(publish&&request.method==='POST'){
    const targetId=decodeURIComponent(publish[1]);await reconcileTargetApplicability(env,targetId);const data=await bundle(env,targetId);if(!data)return json({success:false,error:'평가대상을 찾을 수 없습니다.',code:'TARGET_NOT_FOUND'},404);
