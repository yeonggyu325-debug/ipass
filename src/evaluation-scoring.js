...
export async function handleEvaluationScoring(request,env,ctx,innerApp){
  ...
  if(itemRoute&&request.method==='PATCH'){
    ...
    await env.partner_evaluation_db.batch([
      ...
      env.partner_evaluation_db.prepare(`UPDATE evaluation_targets_v2 SET status=CASE WHEN status='submitted' THEN 'evaluating' ELSE status END,raw_score=(SELECT ROUND(MIN(100,MAX(0,CASE WHEN COALESCE(SUM(CASE WHEN applicable<>0 AND item_type<>'bonus' THEN max_score ELSE 0 END),0)>0 THEN COALESCE(SUM(CASE WHEN applicable<>0 AND item_type<>'bonus' THEN earned_score ELSE 0 END),0)*100.0/SUM(CASE WHEN applicable<>0 AND item_type<>'bonus' THEN max_score ELSE 0 END) ELSE 0 END+COALESCE(SUM(CASE WHEN applicable<>0 AND item_type='bonus' THEN earned_score ELSE 0 END),0))),1) FROM evaluation_target_items_v2 WHERE target_id=?),updated_at=CURRENT_TIMESTAMP WHERE id=?`).bind(targetId,targetId),
    ...
    const data=await bundle(env,targetId);
    return json({success:true,workspace:data});
  }
