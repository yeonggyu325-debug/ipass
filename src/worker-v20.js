...
const COMMON_BEHAVIOR='<script src="/ehs-common.js?v=9"></script>';
...
const SUBMISSION_ASSETS='<link rel="stylesheet" href="/evaluation-submit-enhance.css?v=3"><link rel="stylesheet" href="/evaluation-submit-redesign.css?v=1"><link rel="stylesheet" href="/evaluation-submit-progress.css?v=2"><script src="/evaluation-submit-enhance.js?v=9"></script>';
...
async function injectShared(response,{home=false,root=false,submission=false,embedded=false}={}){
  ...
  html=injectHead(html,COMMON_BEHAVIOR,'/ehs-common.js?v=9');
  if(home)html=injectHead(html,HOME_BOOT,'ehs-home-boot');
  if(root){html=injectBody(html,ROOT_ROUTE_SCRIPT,'ipass-route-v20');html=injectBody(html,PARTNER_ROUTE_SCRIPT,'partner-eval-route-v20');html=injectBody(html,IPASS_GRADE_SCRIPT,'ipass-grade-v21')}
  if(submission)html=injectBody(html,SUBMISSION_ASSETS,'evaluation-submit-enhance.js?v=9');if(embedded)html=injectHead(html,EMBED_STYLE,'ipass-embedded-style');return htmlResponse(response,html)
}
