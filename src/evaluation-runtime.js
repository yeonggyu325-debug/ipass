...
async function policy(env){
  try{const p=await env.partner_evaluation_db.prepare(`SELECT * FROM ipass_policy_settings_v2 WHERE id=1`).first();return {...p,excellent_min:number(p?.excellent_min??p?.excellence_threshold,90),qualified_min:number(p?.qualified_min,70),exemption_threshold:number(p?.exemption_threshold??p?.excellence_threshold,90)}}catch{return {excellent_min:90,qualified_min:70,exemption_threshold:90,first_half_exempt_enabled:1,normal_first_half_weight:40,normal_second_half_weight:40,exempt_second_half_weight:80,committee_weight:10,industrial_accident_weight:10,score_cap:100,bonus_cap:5,manual_publish:1,allow_partner_edits:1,preserve_score_on_edit:1}}
}
async function templateSettings(env,templateId){try{return await env.partner_evaluation_db.prepare(`SELECT * FROM evaluation_template_settings_v2 WHERE template_id=? LIMIT 1`).bind(templateId).first()}catch{return null}}
async function cycleSettings(env,cycleId){try{return await env.partner_evaluation_db.prepare(`SELECT * FROM evaluation_cycle_settings_v2 WHERE cycle_id=? LIMIT 1`).bind(cycleId).first()}catch{return null}}
async function policyForYear(env,year){try{const row=await env.partner_evaluation_db.prepare(`SELECT s.* FROM evaluation_cycle_settings_v2 s JOIN evaluation_cycles_v2 c ON c.id=s.cycle_id WHERE c.year=? ORDER BY CASE WHEN c.half='second' THEN 2 ELSE 1 END DESC,c.created_at DESC LIMIT 1`).bind(year).first();return row||await policy(env)}catch{return policy(env)}}
function normalizeSettings(source={}){return {concept_text:text(source.concept_text,1000)||null,excellent_min:clamp(source.excellent_min??source.excellence_threshold??90),qualified_min:clamp(source.qualified_min??70),first_half_exempt_enabled:source.first_half_exempt_enabled===false||source.first_half_exempt_enabled===0?0:1,exemption_threshold:clamp(source.exemption_threshold??source.excellent_min??source.excellence_threshold??90),normal_first_half_weight:clamp(source.normal_first_half_weight??40),normal_second_half_weight:clamp(source.normal_second_half_weight??40),exempt_second_half_weight:clamp(source.exempt_second_half_weight??80),committee_weight:clamp(source.committee_weight??10),industrial_accident_weight:clamp(source.industrial_accident_weight??10),score_cap:clamp(source.score_cap??100),bonus_cap:clamp(source.bonus_cap??5),manual_publish:source.manual_publish===false||source.manual_publish===0?0:1,allow_partner_edits:source.allow_partner_edits===false||source.allow_partner_edits===0?0:1,preserve_score_on_edit:source.preserve_score_on_edit===false||source.preserve_score_on_edit===0?0:1}}
function validateSettings(s){if(s.qualified_min>=s.excellent_min)return '적격 협력사 최저점은 안전관리 우수 협력사 최저점보다 낮아야 합니다.';if(round1(s.normal_first_half_weight+s.normal_second_half_weight+s.committee_weight+s.industrial_accident_weight)!==100)return '일반 평가 점수 반영 합계는 100점이어야 합니다.';if(round1(s.exempt_second_half_weight+s.committee_weight+s.industrial_accident_weight)!==100)return '면제 평가 점수 반영 합계는 100점이어야 합니다.';return null}
async function putCycleSettings(env,cycleId,templateId,source,userId){const s=normalizeSettings(source),error=validateSettings(s);if(error)return {error};await env.partner_evaluation_db.prepare(`INSERT OR REPLACE INTO evaluation_cycle_settings_v2 (cycle_id,source_template_id,concept_text,excellent_min,qualified_min,first_half_exempt_enabled,exemption_threshold,normal_first_half_weight,normal_second_half_weight,exempt_second_half_weight,committee_weight,industrial_accident_weight,score_cap,bonus_cap,manual_publish,allow_partner_edits,preserve_score_on_edit,updated_by,updated_at) VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,CURRENT_TIMESTAMP)`).bind(cycleId,templateId,s.concept_text,s.excellent_min,s.qualified_min,s.first_half_exempt_enabled,s.exemption_threshold,s.normal_first_half_weight,s.normal_second_half_weight,s.exempt_second_half_weight,s.committee_weight,s.industrial_accident_weight,s.score_cap,s.bonus_cap,s.manual_publish,s.allow_partner_edits,s.preserve_score_on_edit,userId).run();return {settings:await cycleSettings(env,cycleId)}}
...
async function templates(env){
  try{const {results}=await env.partner_evaluation_db.prepare(`SELECT t.*,s.concept_text,s.excellent_min,s.qualified_min,s.first_half_exempt_enabled,s.exemption_threshold,s.normal_first_half_weight,s.normal_second_half_weight,s.exempt_second_half_weight,s.committee_weight,s.industrial_accident_weight,s.score_cap,s.bonus_cap,s.manual_publish,s.allow_partner_edits,s.preserve_score_on_edit,COALESCE((SELECT SUM(i.max_score) FROM evaluation_items_v2 i WHERE i.template_id=t.id AND i.item_type='score'),0) AS score_total,COALESCE((SELECT SUM(i.max_score) FROM evaluation_items_v2 i WHERE i.template_id=t.id AND i.item_type='bonus'),0) AS bonus_total FROM evaluation_templates_v2 t LEFT JOIN evaluation_template_settings_v2 s ON s.template_id=t.id ORDER BY t.year DESC,CASE WHEN t.half='second' THEN 2 ELSE 1 END DESC,t.version DESC`).all();return results||[]}catch{return []}
}
...
async function exemptionCandidates(request,env,ctx,baseWorker,cycle){
  const p=await cycleSettings(env,cycle.id)||await policy(env);if(Number(p?.first_half_exempt_enabled||0)===0)return new Map();const threshold=number(p?.exemption_threshold??p?.excellent_min,90),year=Number(cycle.year);const companies=await activeCompanies(env);const out=new Map();let idx=0;
...
async function ensureTargetRows(request,env,ctx,baseWorker,cycle,userId){
  const companies=await activeCompanies(env);const exemptions=cycle.half==='first'?await exemptionCandidates(request,env,ctx,baseWorker,cycle):new Map();const existingRes=await env.partner_evaluation_db.prepare(`SELECT * FROM evaluation_targets_v2 WHERE cycle_id=?`).bind(cycle.id).all();const existing=new Map((existingRes.results||[]).map(x=>[x.company_id,x]));const stmts=[];
...
async function createCycle(request,env,ctx,baseWorker,payload,user){
  const templateId=text(payload?.template_id,100),t=await templateById(env,templateId);if(!t)return {error:'평가표를 찾을 수 없습니다.',status:404};
  ...
  const year=Math.max(2020,Math.min(2100,Math.round(number(payload?.year,t.year)))),half=payload?.half==='first'?'first':payload?.half==='second'?'second':t.half;
  const existing=await env.partner_evaluation_db.prepare(`SELECT * FROM evaluation_cycles_v2 WHERE year=? AND half=? LIMIT 1`).bind(year,half).first();if(existing)return {error:`${year}년 ${halfLabel(half)} 평가회차가 이미 있습니다. 기존 회차를 선택해 설정을 변경하세요.`,status:409};
  const templatePolicy=await templateSettings(env,t.id)||await policy(env),nextSettings=normalizeSettings({...templatePolicy,...(payload?.settings||{}),concept_text:payload?.concept_text??payload?.settings?.concept_text??templatePolicy?.concept_text}),settingsError=validateSettings(nextSettings);if(settingsError)return {error:settingsError,status:400};
  const id=crypto.randomUUID(),cycleName=text(payload?.cycle_name,200)||`${year}년 ${halfLabel(half)} 이행수준평가`;await env.partner_evaluation_db.prepare(`INSERT INTO evaluation_cycles_v2 (id,year,half,cycle_name,status,template_id,created_by) VALUES (?,?,?,?, 'draft',?,?)`).bind(id,year,half,cycleName,t.id,user.id).run();
  const settingsResult=await putCycleSettings(env,id,t.id,nextSettings,user.id);
  const cycle=await cycleById(env,id);await ensureTargetRows(request,env,ctx,baseWorker,cycle,user.id);await cycleLog(env,id,'cycle_created',{template_id:t.id,year,half,settings:settingsResult.settings},user.id);return {cycle};
}
...
async function updateCycle(request,env,ctx,baseWorker,cycleId,body,user){
  ...
  const start=body.start_at===undefined?cycle.start_at:(text(body.start_at,20)||null),end=body.end_at===undefined?cycle.end_at:(text(body.end_at,20)||null);if(start&&end&&start>end)return {error:'평가 시작일이 종료일보다 늦을 수 없습니다.',status:400};const name=text(body.cycle_name,200)||cycle.cycle_name;
  const hasTargets=Array.isArray(body.targets),rows=hasTargets?await cycleCompanies(request,env,ctx,baseWorker,cycle,user.id):[],by=new Map((hasTargets?body.targets:[]).map(x=>[String(x.company_id),x]));const stmts=[env.partner_evaluation_db.prepare(`UPDATE evaluation_cycles_v2 SET cycle_name=?,start_at=?,end_at=?,updated_at=CURRENT_TIMESTAMP WHERE id=?`).bind(name,start,end,cycle.id)];
  ...
  for(let i=0;i<stmts.length;i+=90)await env.partner_evaluation_db.batch(stmts.slice(i,i+90));if(body.settings){const currentSettings=await cycleSettings(env,cycle.id)||await templateSettings(env,cycle.template_id)||await policy(env),saved=await putCycleSettings(env,cycle.id,cycle.template_id,{...currentSettings,...body.settings},user.id);if(saved.error)return {error:saved.error,status:400}}await cycleLog(env,cycle.id,'cycle_saved',{start_at:start,end_at:end,selected_count:(Array.isArray(body.targets)?body.targets:[]).filter(x=>x.is_selected===true||x.is_selected===1).length},user.id);return {cycle:await cycleById(env,cycle.id)};
}
...
async function adjustedAnnualForCompany(request,env,ctx,baseWorker,companyId,year,usePartnerPath=true){
  ...
  if(!base)return null;
  const p=normalizeSettings(await policyForYear(env,year)),ex=await exemptionRecord(env,companyId,year),firstRaw=await v2RawScore(env,companyId,year,'first'),secondRaw=await v2RawScore(env,companyId,year,'second'),manualFirst=base.first_half_source==='manual',manualSecond=base.second_half_source==='manual',committee=round1(number(base.committee_score,0)*p.committee_weight/10),accident=round1(number(base.industrial_accident_score,0)*p.industrial_accident_weight/10),deduction=number(base.unreasonable_deduction,0);
  if(ex){
    const normalSecond=Math.max(0.1,number(p.normal_second_half_weight,40)),exemptSecond=number(p.exempt_second_half_weight,80),secondBase=manualSecond?base.second_half_score:(secondRaw==null?base.second_half_score:round1(secondRaw*normalSecond/100)),second=secondBase==null?null:round1(Number(secondBase)*exemptSecond/normalSecond),autoSecond=secondRaw==null?(base.auto_second_half_score==null?null:round1(Number(base.auto_second_half_score)*exemptSecond/normalSecond)):round1(secondRaw*exemptSecond/100),final=second==null?null:round1(clamp(second+committee+accident-deduction,0,p.score_cap)),perfect=round1(clamp(exemptSecond+committee+accident-deduction,0,p.score_cap));
    return {...base,first_half_score:null,first_half_source:'exempt',auto_first_half_score:null,second_half_score:second,second_half_source:second==null?null:(manualSecond?'manual':'auto'),auto_second_half_score:autoSecond,first_half_exempt:true,first_half_exempt_reason:ex.exclusion_reason||'전년도 우수협력사',previous_ipass_score:ex.previous_ipass_score,first_half_weight:0,second_half_weight:exemptSecond,committee_score:committee,industrial_accident_score:accident,committee_weight:p.committee_weight,industrial_accident_weight:p.industrial_accident_weight,score_cap:p.score_cap,final_total:final,final_grade:grade(final,p),current_reflected_score:round1(Math.max(0,(second||0)+committee+accident-deduction)),current_reflected_max:second==null?p.committee_weight+p.industrial_accident_weight:p.score_cap,maintain_projection:null,maintain_grade:null,perfect_projection:perfect,perfect_grade:grade(perfect,p),second_half_pending:second==null,grade_settings:{excellent_min:p.excellent_min,qualified_min:p.qualified_min}};
  }
  const firstWeight=number(p.normal_first_half_weight,40),secondWeight=number(p.normal_second_half_weight,40),first=manualFirst?base.first_half_score:(firstRaw==null?base.first_half_score:round1(firstRaw*firstWeight/100)),second=manualSecond?base.second_half_score:(secondRaw==null?base.second_half_score:round1(secondRaw*secondWeight/100)),final=second==null?null:round1(clamp(number(first,0)+number(second,0)+committee+accident-deduction,0,p.score_cap));
  return {...base,first_half_score:first,first_half_source:first==null?null:(manualFirst?'manual':'auto'),second_half_score:second,second_half_source:second==null?null:(manualSecond?'manual':'auto'),auto_first_half_score:firstRaw==null?base.auto_first_half_score:round1(firstRaw*firstWeight/100),auto_second_half_score:secondRaw==null?base.auto_second_half_score:round1(secondRaw*secondWeight/100),first_half_exempt:false,first_half_weight:firstWeight,second_half_weight:secondWeight,committee_score:committee,industrial_accident_score:accident,committee_weight:p.committee_weight,industrial_accident_weight:p.industrial_accident_weight,score_cap:p.score_cap,final_total:final,final_grade:grade(final,p),current_reflected_score:round1(Math.max(0,number(first,0)+number(second,0)+committee+accident-deduction)),current_reflected_max:second==null?firstWeight+p.committee_weight+p.industrial_accident_weight:p.score_cap,second_half_pending:second==null,grade_settings:{excellent_min:p.excellent_min,qualified_min:p.qualified_min}}
}
...
export async function handleEvaluationRuntime(request,env,ctx,baseWorker){
  ...
  if(path.startsWith('/api/admin/evaluation-runtime')){
    ...
    if(request.method==='POST'&&path==='/api/admin/evaluation-runtime/cycles'){const b=await request.json();const r=await createCycle(request,env,ctx,baseWorker,b,user);if(r.error)return json({success:false,error:r.error},r.status);return json({success:true,cycle:r.cycle},201)}
  ...
  if(adminAnnual){const auth=await adminFromRequest(request,env,ctx,baseWorker);if(!auth.ok)return auth.response;const companyId=decodeURIComponent(adminAnnual[1]),year=Number(adminAnnual[2]);if(request.method==='GET'){const a=await adjustedAnnualForCompany(request,env,ctx,baseWorker,companyId,year,false);return a?json({success:true,annual:a}):null}if(request.method==='PATCH'){const ex=await exemptionRecord(env,companyId,year);let body=await request.json();if(ex){const p=normalizeSettings(await policyForYear(env,year)),effectiveMax=p.exempt_second_half_weight,normalMax=Math.max(.1,p.normal_second_half_weight);body={...body};delete body.first_half_mode;delete body.first_half_score;if(body.second_half_mode==='manual'){const effective=Number(body.second_half_score);if(!Number.isFinite(effective)||effective<0||effective>effectiveMax)return json({success:false,error:`면제 연도 하반기 점수는 0~${effectiveMax}점으로 입력하세요.`},400);body.second_half_score=round1(effective*normalMax/effectiveMax)}}const r=await baseWorker.fetch(new Request(request.url,{method:'PATCH',headers:request.headers,body:JSON.stringify(body)}),env,ctx);if(!r.ok)return r;const a=await adjustedAnnualForCompany(request,env,ctx,baseWorker,companyId,year,false);return json({success:true,annual:a})}}
