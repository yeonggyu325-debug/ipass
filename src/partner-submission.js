...
async function saveItemsBulk(env,access,body){
  ...
  const {results}=await env.partner_evaluation_db.prepare(`SELECT id,description,applicable,earned_score FROM evaluation_target_items_v2 WHERE target_id=?`).bind(access.target.target_id).all();const rows=new Map((results||[]).map(row=>[row.id,row]));
...
async function submissionSummary(env,targetId){
  const row=await env.partner_evaluation_db.prepare(`SELECT COUNT(*) AS total,SUM(CASE WHEN applicable<>0 THEN 1 ELSE 0 END) AS applicable,SUM(CASE WHEN applicable=0 THEN 1 ELSE 0 END) AS na,SUM(CASE WHEN applicable<>0 AND (TRIM(COALESCE(description,''))<>'' OR EXISTS (SELECT 1 FROM evaluation_evidence_files_v2 f WHERE f.target_item_id=evaluation_target_items_v2.id AND f.deleted_at IS NULL)) THEN 1 ELSE 0 END) AS prepared FROM evaluation_target_items_v2 WHERE target_id=?`).bind(targetId).first();
