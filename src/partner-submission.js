...
async function saveItem(env,access,itemId,body){
  const row=await env.partner_evaluation_db.prepare(`SELECT id,description,applicable,earned_score FROM evaluation_target_items_v2 WHERE id=? AND target_id=? LIMIT 1`).bind(itemId,access.target.target_id).first();
  if(!row)return {error:'평가항목을 찾을 수 없습니다.',status:404};
  if(Number(row.applicable)===0)return {error:'N/A 항목은 제출자료를 입력할 수 없습니다.',status:409};
  const description=clean(body.description,4000),changed=String(row.description||'')!==description;
  await env.partner_evaluation_db.prepare(`UPDATE evaluation_target_items_v2 SET description=?,updated_at=CURRENT_TIMESTAMP WHERE id=?`).bind(description||null,row.id).run();
  await env.partner_evaluation_db.prepare(`UPDATE evaluation_targets_v2 SET status=CASE WHEN status='not_started' THEN 'in_progress' ELSE status END,updated_at=CURRENT_TIMESTAMP WHERE id=?`).bind(access.target.target_id).run();
  if(changed&&access.target.submitted_at)await markPartnerChange(env,access.target.target_id,row.id,false);
  await log(env,access.target.target_id,row.id,access.target.submitted_at?'post_submit_item_edit':'item_saved',{before_description:row.description||'',after_description:description,needs_rescore:changed&&row.earned_score!==null&&row.earned_score!==undefined},access.user.id);
  return {ok:true,saved_at:new Date().toISOString()};
}

async function submit(env,access){const t=await target(env,access.target.target_id);if(!clean(t.business_number,30)||!clean(t.representative_name,100)||t.worker_count===null||t.worker_count===undefined)return {error:'제출 전 회사 기본정보를 먼저 저장하세요.',status:400};const w=await workspace(env,t);const resubmitted=!!t.submitted_at;await env.partner_evaluation_db.prepare(`UPDATE evaluation_targets_v2 SET status=CASE WHEN status IN ('not_started','in_progress','submitted') THEN 'submitted' ELSE status END,submitted_at=COALESCE(submitted_at,CURRENT_TIMESTAMP),updated_at=CURRENT_TIMESTAMP WHERE id=?`).bind(t.target_id).run();await log(env,t.target_id,null,resubmitted?'resubmitted':'submitted',{prepared:w.summary.prepared,blank:w.summary.blank,applicable:w.summary.applicable},access.user.id);return {ok:true,resubmitted,summary:w.summary,submitted_at:t.submitted_at||new Date().toISOString()}}

async function addFile(request,env,access,itemId){if(!env.EVIDENCE_FILES)return {error:'증빙자료 저장소가 아직 연결되지 않았습니다.',status:503,storage_available:false};const item=await env.partner_evaluation_db.prepare(`SELECT id,applicable,earned_score FROM evaluation_target_items_v2 WHERE id=? AND target_id=? LIMIT 1`).bind(itemId,access.target.target_id).first();if(!item)return {error:'평가항목을 찾을 수 없습니다.',status:404};if(Number(item.applicable)===0)return {error:'N/A 항목에는 증빙자료를 첨부할 수 없습니다.',status:409};const form=await request.formData();const file=form.get('file');if(!(file instanceof File)||!file.name)return {error:'첨부할 파일을 선택하세요.',status:400};if(file.size<=0||file.size>MAX_FILE_BYTES)return {error:'파일은 25MB 이하만 첨부할 수 있습니다.',status:400};const ext=(file.name.split('.').pop()||'').toLowerCase();if(!ALLOWED_EXT.has(ext))return {error:'PDF, JPG, PNG, Excel, HWP, PowerPoint, Word 파일만 첨부할 수 있습니다.',status:400};const id=crypto.randomUUID(),name=safeFileName(file.name),key=`evaluations/${access.target.year}/${access.target.half}/${access.target.company_id}/${access.target.target_id}/${itemId}/${id}-${name}`;await env.EVIDENCE_FILES.put(key,file.stream(),{httpMetadata:{contentType:file.type||'application/octet-stream'},customMetadata:{originalName:name,uploadedBy:String(access.user.id||'')}});await env.partner_evaluation_db.prepare(`INSERT INTO evaluation_evidence_files_v2 (id,target_id,target_item_id,object_key,file_name,content_type,file_size,uploaded_by) VALUES (?,?,?,?,?,?,?,?)`).bind(id,access.target.target_id,itemId,key,name,file.type||'application/octet-stream',file.size,access.user.id).run();if(access.target.submitted_at)await markPartnerChange(env,access.target.target_id,itemId,false);await log(env,access.target.target_id,itemId,access.target.submitted_at?'post_submit_file_added':'file_added',{file_id:id,file_name:name,file_size:file.size,needs_rescore:item.earned_score!==null&&item.earned_score!==undefined},access.user.id);return {ok:true,file:{id,target_item_id:itemId,file_name:name,content_type:file.type||'application/octet-stream',file_size:file.size,created_at:new Date().toISOString()}}}
