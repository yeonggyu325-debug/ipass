...
const ONE_SHOT_REQUEST_ID='5f03af5f-f04b-4ffe-988b-b5fc83ab4b96';
...
export async function recordRequestAudit(env,{requestId,method,path,status,durationMs,actorId=null,actorRole=null}){
  try{await env.partner_evaluation_db.prepare(`INSERT INTO system_request_audit_v2 (id,request_id,method,path,status,duration_ms,actor_id,actor_role) VALUES (?,?,?,?,?,?,?,?)`).bind(crypto.randomUUID(),requestId,method,path,status,durationMs,actorId,actorRole).run()}catch(error){console.error('request audit write failed',error)}
}
...
export async function handleSystemAdmin(request,env,ctx,innerApp){
  ...
  if(request.method==='GET'&&path==='/api/admin/system/diagnostics'){
    ...
    let dbError=null;let rows=[];try{const result=await env.partner_evaluation_db.prepare(`SELECT name FROM sqlite_master WHERE type='table' AND name IN (${REQUIRED_TABLES.map(()=>'?').join(',')})`).bind(...REQUIRED_TABLES).all();rows=result.results||[];checks.d1=true}catch(error){dbError=String(error?.message||error)}
  ...
  if(request.method==='GET'&&path==='/api/admin/system/database'){
    const [indexResult,templateResult,cycleResult,targetResult,itemResult,fileResult]=await env.partner_evaluation_db.batch([
      ...
      env.partner_evaluation_db.prepare(`SELECT COUNT(*) AS count FROM evaluation_templates_v2`),env.partner_evaluation_db.prepare(`SELECT COUNT(*) AS count FROM evaluation_cycles_v2`),env.partner_evaluation_db.prepare(`SELECT COUNT(*) AS count FROM evaluation_targets_v2`),env.partner_evaluation_db.prepare(`SELECT COUNT(*) AS count FROM evaluation_target_items_v2`),env.partner_evaluation_db.prepare(`SELECT COUNT(*) AS count,COALESCE(SUM(file_size),0) AS bytes FROM evaluation_evidence_files_v2 WHERE deleted_at IS NULL`)
  ...
  if(request.method==='GET'&&path==='/api/admin/system/requests'){
    const limit=Math.min(200,Math.max(1,Number(url.searchParams.get('limit')||50))),errorsOnly=url.searchParams.get('errors')==='1',clause=errorsOnly?'WHERE status >= 400':'';const {results}=await env.partner_evaluation_db.prepare(`SELECT request_id,method,path,status,duration_ms,actor_id,actor_role,created_at FROM system_request_audit_v2 ${clause} ORDER BY created_at DESC LIMIT ?`).bind(limit).all();return json({success:true,requests:results||[],filter:{errors_only:errorsOnly,limit}})
  }
  if(request.method==='GET'&&path==='/api/admin/system/submission-activity'){
    const limit=Math.min(200,Math.max(1,Number(url.searchParams.get('limit')||50))),postSubmitOnly=url.searchParams.get('post_submit')!=='0';const exists=await env.partner_evaluation_db.prepare(`SELECT name FROM sqlite_master WHERE type='table' AND name='evaluation_partner_submission_logs_v2'`).first();if(!exists)return json({success:true,activities:[],filter:{post_submit_only:postSubmitOnly,limit}});const clause=postSubmitOnly?`AND l.action IN ('post_submit_profile_edit','post_submit_item_edit','post_submit_file_added','file_deleted','resubmitted')`:'';const {results}=await env.partner_evaluation_db.prepare(`SELECT l.action,l.target_id,l.target_item_id,l.detail_json,l.changed_by,l.created_at,c.company_name,ec.year,ec.half,et.status AS target_status FROM evaluation_partner_submission_logs_v2 l JOIN evaluation_targets_v2 et ON et.id=l.target_id JOIN companies c ON c.id=et.company_id JOIN evaluation_cycles_v2 ec ON ec.id=et.cycle_id WHERE 1=1 ${clause} ORDER BY l.created_at DESC LIMIT ?`).bind(limit).all();return json({success:true,activities:results||[],filter:{post_submit_only:postSubmitOnly,limit}})
  }
