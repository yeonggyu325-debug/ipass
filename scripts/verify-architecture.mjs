...
const shell=await readFile(new URL('../public/portal-shell-v1.css',import.meta.url),'utf8');
...
assert.ok(worker.includes("'/admin/approvals'")&&worker.includes("'/admin/accounts'"),'legacy dedicated admin routes must remain compatible');
assert.ok(toolbar.includes('/admin/approvals')&&toolbar.includes('협력사 관리센터'),'toolbar must use the unified partner administration entry');
assert.ok(entry.includes("path==='/admin/partners'")&&entry.includes("rewritePath(request,'/admin/accounts')"),'unified partner route must reuse the admin workspace');
...
console.log(JSON.stringify({success:true,consolidated_worker:true,html_injection_guard:true,unified_partner_admin:true,ui_foundation:'v3',ui_asset_injection:'worker-only',submission_css:'single-file',legacy_files_removed:removed.length}));