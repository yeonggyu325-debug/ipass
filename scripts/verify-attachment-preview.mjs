...
const [viewer,education,educationApi,partnerApi,worker,resourcePreview,resourcePreviewCss,packageJson,assetBuilder]=await Promise.all([
...
if(!worker.includes('attachment-preview.js?v=4'))failures.push('worker:viewer-injection');
...
if(!worker.includes("path==='/resources'"))failures.push('resource-preview:route-scope');
if(!worker.includes('/resource-preview-v2.css?v=2'))failures.push('resource-preview:css-injection');
if(!worker.includes('/resource-preview-v2.js?v=2'))failures.push('resource-preview:js-injection');
...
console.log(JSON.stringify({
  ...
  resource_preview:{route:'/resources',aspect_ratio_preserved:true,viewport_fit:true,enterprise_ui:true}
}));
