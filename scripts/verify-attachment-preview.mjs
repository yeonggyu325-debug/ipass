...
if (!worker.includes('/resource-preview-v2.js?v=5')) failures.push('resource-preview:v2-cache-bust');
if (!worker.includes('/resource-preview-v3.js?v=5')) failures.push('resource-preview:v3-cache-bust');

if (failures.length) throw new Error(`Attachment preview verification failed: ${failures.join(', ')}`);
console.log(JSON.stringify({success:true,browser_renderers:['pdf','xlsx','docx','pptx','hwp','hwpx','image'],web_viewer_fallbacks:['ppt','doc','hwp-on-error','hwpx-on-error'],allowed_extensions:requiredExtensions.length,consolidated_worker:true,resource_preview:{route:'/resources',single_injection_source:true,aspect_ratio_preserved:true,initial_fit_only:true,wheel_zoom:true,docx_wheel_zoom:true,xlsx_wheel_zoom:true,pdf_native_rerender_zoom:true,pptx_native_renderer_zoom:true,editable_zoom:true,legacy_zoom_controls_hidden:true,legacy_original_ratio_label:false,enterprise_ui:true}}));
