...
assert.match(page, /직급·성명은 관리자가 기재합니다/);
...
assert.match(page, /관리자 승인 전에는 불참으로 처리됩니다/);
assert.match(page, /id="draftSaveBtn"/);
assert.match(page, /회의 확정/);
...
assert.match(handler, /관리자가 직급과 성명을 먼저 입력한 뒤 위임장을 제출할 수 있습니다/);
