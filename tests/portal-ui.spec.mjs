...
async function assertNoCodeLeak(page){
  ...
  expect(text).not.toContain('window.after=true');
}
...
test('관리자는 신규 홈과 모든 관리자 경로를 일관되게 사용한다',async({page})=>{
  ...
  await expect(page.locator('#ehsGlobalNav .ehs-global-link')).toHaveCount(8);
  await expect(page.locator('#ehsGlobalName')).toHaveText('관리자 부장');
  await expect(page.locator('body')).not.toContainText('admin@example');
